...
if __name__ == '__main__':
    ...
    argparser.add_argument(
        "-p",
        "--phonenumber",
        default=master_phone_number,
        help="enter phone number to lookup")
    argparser.add_argument(
        "-a",
        "--accountsid",
        default=master_account_sid,
        help="enter account sid for lookup")
    argparser.add_argument(
        "-t",
        "--authtoken",
        default=master_auth_token,
        help="enter auth token for lookup")
    argparser.add_argument(
        "-s",
        "--servicelevel",
        default=master_service_level,
        help="enter service level for lookup")
    argparser.add_argument(
        "-o",
        "--opencnamurl",
        default=master_opencnam_url,
    ...
    argparser.add_argument(
        ...
        "--input",
        action="store_false",
        help="get input from command prompt")
...
def make_http_headers_from_dict_to_list(
    headers={
...
def query_cnam_info(
        phone_number=master_phone_number,
        account_sid=master_account_sid,
        auth_token=master_auth_token,
        opencnam_url=master_opencnam_url,
        service_level=master_service_level,
        casing=master_casing,
        mobile=master_mobile,
        no_value=master_no_value,
        geo=master_geo,
        httplibuse=master_http_lib):
    if (phone_number is None or account_sid is None or auth_token is None or service_level is None or (
            service_level != "standard" and service_level != "plus")):
        return False
    if (not get_httplib_support(httplibuse)):
        httplibuse = "urllib"
    if (httplibuse == "urllib"):
        opencnam_api_url = Request(
            opencnam_url.format(
                phone_number_str=phone_number,
                account_sid_str=account_sid,
                auth_token_str=auth_token,
                service_level_str=service_level,
                casing_str=casing,
                mobile_str=mobile,
                no_value_str=no_value,
                geo_str=geo))
        preb64_user_string = str(master_account_sid) + \
        ...
        r_header.update(
            {'Referer': "https://www.opencnam.com/dashboard/delivery/query-tool"})
        opencnam_api_data = requests.get(
            opencnam_url.format(
                phone_number_str=phone_number,
                account_sid_str=account_sid,
                auth_token_str=auth_token,
        ...
        print(r_header)
        opencnam_api_data = geturls_opener.open(
            opencnam_url.format(
                phone_number_str=phone_number,
                account_sid_str=account_sid,
                auth_token_str=auth_token,
